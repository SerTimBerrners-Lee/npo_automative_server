...
export class SettingsService {
    ...
    async getAllPodTypeMaterial(instans: string | number) {
        ...
        if (instans == 'all')
            ...
            materials = await this.podMaterialReprository.findAll({where: { instansMaterial: instans, ban: false }});
    ...
    async removeMaterial(id: number) {
        ...
        if (!material)
            throw new HttpException('Материал не найден', HttpStatus.NOT_FOUND);
        const result = await this.materialReprository.findByPk(id);
        if (result) {
    ...
    async createPodMaterial(dto: CreatePodMaterialDto) {
        ...
        if (density && density.edizm) {
            const res = await this.edizmReprository.findByPk(density.edizm);
            if (res) pod_material.density = JSON.stringify({edizm: res, znach: density.znach});
        }
    ...
    async removePodMaterial(id: number) {
        const result = await this.podMaterialReprository.findByPk(id);
        if (result) {
            result.ban = !result.ban;
            await result.save();
        }
    ...
    async updatePodTypeMaterial(dto: CreatePodMaterialDto) {
        ...
        else pod_material.density = null;
    ...
    async createAndUpdatePodPodMaterial(dto: CreatePodPodMaterial, files: any) {
        ...
        if (length && length.edizm && length.znach) 
            await this.edizmReprository.findByPk(length.edizm).then(res => {
                if (res) podPodMaterial.length = JSON.stringify({edizm: res, znach: length.znach})
            })
        else 
        ...
        if (height && height.edizm && height.znach) 
            await this.edizmReprository.findByPk(height.edizm).then(res => {
                if (res) podPodMaterial.height = JSON.stringify({edizm: res, znach: height.znach})
            })
        else 
        ...
        if (dto.rootParentId) {
            ...
            if (dto.rootParentId) { 
                const material = await this.materialReprository.findByPk(dto.rootParentId);
    ...
    async getAllShipmentsPPM() {
        ...
        for (const mat of materials) {
            if (mat.deliveries && mat.deliveries.length) {
                for (const dev of mat.deliveries) {
    ...
    async getAllMaterialProviderById(id: number) {
        ...
        const new_materials = [];
        for (const material of material_providers) {
            for(let provider of material.providers) {
                if(provider.id == id)  new_materials.push(material);
            }
        }
